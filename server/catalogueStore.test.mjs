...
test('a running two-pass search keeps its captured catalogue when refresh completes', async t => {
  ...
  assert.match(calls[1].instructions, /autre modalité pertinente/)
  assert.match(calls[1].instructions, /e-learning, le distanciel, le présentiel/)
  assert.match(calls[1].instructions, /ne les présente pas comme un parcours cumulatif/)
  assert.match(calls[1].instructions, /demande générale et un niveau non précisé/)
